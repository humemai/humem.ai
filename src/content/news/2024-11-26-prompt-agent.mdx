---
title: "Introducing PromptAgent for text-to-graph and graph-to-text"
...
HumemAI is introducing **PromptAgent**, an AI system that uses **LLM prompting** to convert text into knowledge graphs (text2graph) and reconstruct structured graphs back into textual descriptions (graph2text).
...
This approach keeps PromptAgent scalable by relying on **graph-based memory retrieval** instead of holding an entire dataset in context, allowing it to handle larger knowledge bases more efficiently.
...
HumemAI will continue to share updates as PromptAgent evolves.