  ...
  subprojectPage: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          "The same harness runs operational and analytical work over documents, in OLTP and OLAP shapes, time series on TSBS, the Time Series Benchmark Suite, and a cross-model transaction that starts from a vector hit, traverses the graph, and updates a document. The tables below and the summary figure at the end all use compressed labels, which are worth having to hand:",
          "| Label | What it means |\n| --- | --- |\n| Txn | Transaction. |\n| OLTP | Online transaction processing: many small reads and writes, counted in operations per second. |\n| OLAP | Online analytical processing: a few large scanning queries, timed in milliseconds. Its rows are in the tables further down. |\n| TS | Time series. Agg is an aggregation over a window; pts/s is points ingested per second. |\n| TPC-H Q1 | The first query of a long-standing analytical benchmark. |\n| Sparse, Dense | The two kinds of vector above. The number beside each is the corpus size. |",
          {
            type: "benchmarkTable",
            tableId: "l1",
            caption: "Transactional and analytical work over documents, against the relational specialists PostgreSQL and DuckDB. ArcadeDB stores documents, not tables, and answers the same SQL.",
          },
          {
            type: "benchmarkTable",
            tableId: "l1olap",
          ...
          "One figure for all of it. Every row above appears here as a ratio against the strongest comparator for that workload.",
          {
            ...
            columns: 1,
            caption:
              "The whole evaluation in one figure, not just the vector part: ArcadeDB embedded against the best specialist on each workload, log scale, anything right of the line a win. It wins the operational and cross-model rows, matches DuckDB on time-series ingest, and loses every scan-, bulk- and search-bound row, which is the honest shape for a general engine measured against specialists. It does not win vector search. Every bar is a first timed pass after the index is built, on both sides, and at ten million vectors that is 8.75 ms for ArcadeDB against Qdrant's 1.26 and Chroma's 0.70. The dense bar divides by Qdrant rather than by Chroma, which the table shows is faster, because Chroma returns 93.4% of the true neighbours where ArcadeDB returns 95.3%: each row compares us against one specialist, chosen this way: of the engines that find at least as many of the true nearest neighbours as we do, the fastest. Comparing against a quicker engine that finds fewer would not be a fair race. The pass matters only for us: run the same query set again and ArcadeDB answers in 1.04 ms, because it pages its index off disk and the second pass finds it resident, while Qdrant moves to 1.25 and Chroma to 0.72. So the dense row shows both. The bar is the first pass and the diamond is the same comparison in steady state, a 1.2x win, with the comparators where they were. No other row can show both, because every other lane times a single pass. The cross-model row is measured against SurrealDB, which is the fastest engine here that also does the whole operation in one transaction, rather than against the composed stack, which is slower and has no transaction spanning its two engines at all. The rows use each engine's own idiom: document OLTP is SQL against PostgreSQL, the graph row is Cypher against LadybugDB, and TPC-H Q1 is SQL against DuckDB.",
            items: [
