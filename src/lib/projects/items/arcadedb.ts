    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          ...
          "Every corpus here is real and published, never generated, and the sparse case shows why that matters. A SPLADE (Sparse Lexical And Expansion model) vector holds one weight per vocabulary term and is almost all zeros, so what a query costs depends on which terms it happens to hit: a common word carries a far longer posting list than a rare one. Real text is lopsided that way and synthetic data is not, which flatters an approximate index. Dense search uses published image descriptors for the same reason.",
