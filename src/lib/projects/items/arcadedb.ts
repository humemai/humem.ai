...
export const arcadeDb: Project = {
  ...
  subprojectPage: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          "The same harness runs OLTP and OLAP work over documents, time series on TSBS, the Time Series Benchmark Suite, and a cross-model transaction that starts from a vector hit, traverses the graph, and updates a document. The tables below and the summary figure at the end use short labels:",
          ...
          "One figure for all of it. Every row above appears here as a ratio against the strongest comparator on that workload.",
          {
            type: "figureGrid",
            ...
            columns: 1,
            caption:
              "The whole evaluation in one figure: ArcadeDB embedded against the best comparator on each workload, log scale, anything right of the line a win. It wins the OLTP and cross-model rows, matches DuckDB on time-series ingest, and loses every scan-, bulk- and search-bound row, including vector search. Every bar is cold on both sides, and at ten million vectors that is 8.75 ms for ArcadeDB against Qdrant's 1.26 and Chroma's 0.70. The dense bar divides by Qdrant rather than Chroma, which the table shows is faster, because Chroma returns 93.4% of the true neighbours where ArcadeDB returns 95.3%. Each row picks its comparator the same way: of the engines that find at least as many true neighbours as ArcadeDB, the fastest. Warm matters only for ArcadeDB: run the same queries again and ArcadeDB answers in 1.04 ms, because it pages its index off disk and the second pass finds it resident, while Qdrant moves to 1.25 and Chroma to 0.72. So the dense row shows both: the bar is cold and the diamond is warm, a 1.2x win, with the comparators where they were. No other row can show both, because every other lane times one pass. The cross-model row is measured against SurrealDB, the fastest engine here that also does the whole operation in one transaction, rather than the composed stack, which is slower and has no transaction spanning its two engines. Each row uses the comparator's own language: document OLTP is SQL against PostgreSQL, the graph row is Cypher against LadybugDB, and TPC-H Q1 is SQL against DuckDB.",
            items: [
  ...
  summary:
    "A multi-model database engine that keeps documents, graphs, key-value, time series and vectors in one transactional engine, plus the Python package that runs that engine inside your process, benchmarked against the comparators in each category.",
  image: {
