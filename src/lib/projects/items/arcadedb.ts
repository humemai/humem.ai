    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          ...
          "What is not here is as deliberate as what is. A time-series lane against QuestDB and DuckDB has been run and is reported in the papers, including a measured decision not to give any engine a settle step, since settling only ours would have taken a one-sided advantage. It is not on this page yet for a duller reason: that lane is driven by its own probe rather than the shared runner, so its rows do not carry the scale, workload and comparator builds that every other row here carries, and this page publishes only rows that can state their own conditions. It joins the tables once the probe emits the same envelope as the rest. The recovery and replication work is reported as prose rather than as a table, since a kill-9 either recovers or it does not.",
