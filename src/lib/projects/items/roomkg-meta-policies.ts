		...
		sections: [
			...
			},
			{
				...
				body: [
					...
					"A small worked example shows why the choice matters. Suppose memory holds `(sarah, at_location, living)` with annotations $(4, 11, 5)$ and `(sarah, at_location, kitchen)` with $(10, 10, 1)$. For the query \"where is sarah?\", MRA answers `kitchen` because that fact was added most recently, while MRU and MFU answer `living`. If Sarah has just moved, only MRA is correct. No single fixed rule wins everywhere, which is exactly why selection should be learned.",
