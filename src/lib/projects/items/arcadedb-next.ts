    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          ...
          "The triangle count was written off as beyond SurrealDB's query language, and that was wrong: it was untried rather than impossible. It is written now for both of that engine's deployments and checked against the same count every other engine returns, and where a row is missing from the table below, the table says which cell ran out of time rather than leaving the gap to be guessed at. The two deployments run different text for it, because they are two different versions of the engine and each plans one spelling better than the other, by a factor large enough that a single shared text would be measuring our phrasing rather than either version. Both spellings are kept in the harness so the choice can be checked, and the answer check is what proves they ask the same question.",
          {
            ...
            tableId: "l2olap",
            caption:
              "Average friend age, friendships within a city, most friends, the degree distribution, and the triangle count. The triangle count is the cell most likely to exceed its time budget at the larger size; a cell that does is published as censored, with its budget, rather than dropped.",
          },
        ],
      },
      {
      ...
      },
      {
        ...
        body: [
          ...
          "The table carries every timed write rather than one representative each, and read down one engine it says something the single row could not. Whatever an operation costs when the commit does not wait, on the same engine it converges on roughly the same number when it does, because each one pays a single flush and a flush costs what a flush costs. Two things follow from that, and the second is the one worth having. The ratio is inverse to how fast the relaxed path was, so the engines that look worst in the last column are the ones that were quickest without the wait rather than the slowest with it. And a transaction that touches several records pays the same single tax as a one-record write, which is what makes this a cost per commit and not a cost per operation. The read is on the table as the control and is the shortest way to see it: it runs in both cells, it commits nothing, and it does not move.",
